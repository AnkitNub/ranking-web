...
//
// Boundary behavior: when the *last* judge for a participant has just scored
// (or timed out), we don't advance to the next participant. We park the event
// in 'interlude' so the presentation can run that participant's breakdown +
// leaderboard re-sort animations. Admin leaves interlude via leaveInterlude().
async function advanceOnce({ event }) {
  const pIdx = event.current_participant_index;
  const jIdx = event.current_judge_index + 1;
...
// Leaves interlude → starts the next participant's first judge turn, or ends
