...
export async function recordScore({
  ...
  // Validate decimal places: reject scores that are more precise than allowed
  const multiplier = Math.pow(10, decimalPlaces);
  const rounded = Math.round(numScore * multiplier) / multiplier;
  if (Math.abs(rounded - numScore) > 1e-9)
    return { error: 'bad_score_precision' };
